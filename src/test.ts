...
const cypher = ({ e = null, d = null, t = null }): string => {
  ...
  const output = instance.run() as OutputObj;

  if (e) return output.encoded;
  else return output.decoded;
};

console.log(
  cypher({
    e:
      "toto je velmi obycejna zprava dokonce tak obycejna ze nikoho nezajima je hezky pocasi a mame se tu dobre",
    t: testTable2,
  })
);
...
console.log(
  cypher({
    d:
      "UFAFB ZKRCL SFOJB PKEOR HUYDS GNBNB BPVFD NOJBP KEORZ KOXNW FBWJY DYLSD ZJPJE JIYBF GOFTS TRVWV PGNOV ZVTRW",
    t: testTable2,
  })
);
...
console.log(
  cypher({
    d:
      "FZCLX OOYLD OAMSB BOGFT XNSNW GXZGJ OXIOD MFZVZ KOAMS BBOGF TXNSN WGXZG JOGFT LFZVZ VSBBO DJJCZ PVYWG FIOMM",
    t: testTable2,
  })
);
