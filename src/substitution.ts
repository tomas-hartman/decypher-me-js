...
const analyzeSegments = (str: string, isLetters = true) => {
...
const decypher = (str: string, key1: string[][], key2: string[][]) => {
...
const analyzeDuplets = (str: string): string[][] => {
  const text = cleanText(str);
 ...
 */
const encode = (textToEncode: string, key1?: string, key2?: string): string => {
  ...
  for (let i = 0; i < text.length; i++) {
    const letter = text[i];
    const index = alphabet1.findIndex(
      (itemFromAlphabet) => itemFromAlphabet === letter
    );
...
console.log("\nENCODED TEXT");
console.log(
  encode("Ahoj Barborko jakpak se máš zlatíčko moje?", myKey, myKey2)
);
